 ...
 void timerIsr(void)
{
  ...
  //Left Motor Speed 
  right_wheel1_vel.data = float(counter_right1)*2*pi*r/90;
  right_wheel1_vel_pub.publish(&right_wheel1_vel);
  left_wheel2_vel.data = float(counter_left2)*2*pi*r/90;
  left_wheel2_vel_pub.publish(&left_wheel2_vel);
  left_wheel3_vel.data = float(counter_left3)*2*pi*r/90;
  left_wheel3_vel_pub.publish(&left_wheel3_vel);
  right_wheel4_vel.data = float(counter_right4)*2*pi*r/90;
