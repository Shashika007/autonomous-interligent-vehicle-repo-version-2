 ...
 void timerIsr(void)
{
  ...
  //Left Motor Speed 
  right_wheel1_vel.data = float(counter_right1)*2*pi*r/2000;  // 2*pi*r/1000 = 0.0471
  right_wheel1_vel_pub.publish(&right_wheel1_vel);
  left_wheel2_vel.data = float(counter_left2)*0.0471;
  left_wheel2_vel_pub.publish(&left_wheel2_vel);
  left_wheel3_vel.data = float(counter_left3)*2*pi*r/1000;
  left_wheel3_vel_pub.publish(&left_wheel3_vel);
  right_wheel4_vel.data = float(counter_right4)*2*pi*r/1000;
