...
# Use a PID that compares the error based on sensed velocity by encoder readings
class ControlsToXYTH:
  def __init__(self):
    rospy.init_node('aiv_controller')
    ...
    self.motorMec_max_angular_vel = rospy.get_param('~motorMec_max_angular_vel',5.32)
    # Wheel can turn ~6 ticks per second which is approx 5.34 rad / s when motor_cmd = 125
    self.motorMec_min_angular_vel = rospy.get_param('~motorMec_min_angular_vel',1.28)
    # Corresponding motor commands
    self.motorMec_cmd_max = rospy.get_param('~motorMec_cmd_max',255)
    self.motorMec_cmd_min = rospy.get_param('~motorMec_cmd_min',110)

    self.R = rospy.get_param('~Mec_wheel_radius', 0.03)
    self.pid_on = rospy.get_param('~pid_on',True)

    # (Optional) Publish the computed angular velocity targets
    self.wheel_right1_angular_vel_target_pub = rospy.Publisher('wheel_right1_angular_vel_target', Float32, queue_size=10)
    self.wheel_left2_angular_vel_target_pub = rospy.Publisher('wheel_left2_angular_vel_target', Float32, queue_size=10)
    self.wheel_left3_angular_vel_target_pub = rospy.Publisher('wheel_left3_angular_vel_target', Float32, queue_size=10)
    self.wheel_right4_angular_vel_target_pub = rospy.Publisher('wheel_right4_angular_vel_target', Float32, queue_size=10)

    # (Optional) Publish the computed angular velocity control command
    self.wheel_right1_angular_vel_control_pub = rospy.Publisher('wheel_right1_angular_vel_control', Float32, queue_size=10)
    self.wheel_left2_angular_vel_control_pub = rospy.Publisher('wheel_left2_angular_vel_control', Float32, queue_size=10)
    self.wheel_lett3_angular_vel_control_pub = rospy.Publisher('wheel_left3_angular_vel_control', Float32, queue_size=10)
    self.wheel_right4_angular_vel_control_pub = rospy.Publisher('wheel_right4_angular_vel_control', Float32, queue_size=10)
    ...
    # Read in sensor velocity from encoder for PID control
    self.robot_x_vel_enc_sub = rospy.Subscriber('robot_x_angular_vel_enc', Float32, self.robot_x_angular_vel_enc_callback)
    self.robot_y_vel_enc_sub = rospy.Subscriber('robot_y_angular_vel_enc', Float32, self.robot_y_angular_vel_enc_callback)
    self.robot_angular_vel_enc_sub = rospy.Subscriber('robot_angular_vel_enc', Float32, self.robot_angular_vel_enc_callback)

    # Read in velocity targets
