...
class ControllerClient{
  ...
  public:
    ...
    ControllerClient(std::string name):
	    ...
	    //Set up the client. It's publishing to topic "lqr_control", and is set to auto-spin
	    ac("lqr_control", true),
...
void doneCb(const actionlib::SimpleClientGoalState& state, const aiv_controller_lqr::lqrResultConstPtr& result)
{
...
void feedbackCb(const aiv_controller_lqr::lqrFeedbackConstPtr& feedback)
{
	ROS_INFO("Got Feedback of Progress to Goal: position: %f", feedback->x_dot);
};

void GoalCallback(const geometry_msgs::Twist& twist)
{

	goal.x_dot =twist.linear.x;
	goal.y_dot =twist.linear.y;
	goal.th_dot =twist.angular.z;
...
private:
	actionlib::SimpleActionClient<aiv_controller_lqr::lqrAction> ac;
	std::string action_name;
	aiv_controller_lqr::lqrGoal goal;
